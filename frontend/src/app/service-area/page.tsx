...
export const metadata = { title: "Service Area — Indiana & Illinois" };
export const revalidate = 3600;

async function getCounties() {
  const r = await fetch(`${API}/public/service-area`, { next: { revalidate: 3600 } });
  return r.json();
}

export default async function ServiceAreaPage() {
  const { counties } = await getCounties();
  const grouped: Record<string, any[]> = { IN: [], IL: [] };
